...
@Repository("EmissionWorkPlaceAirDao")
public class EmissionWorkPlaceAirDao extends AbstractCommonJdbcDao {
	...
	public DataTableAjax<EmissionWorkPlaceAir> getDataTable(EmissionWorkPlaceAir param){
		...
		sql.append(" SELECT ewa.* FROM she_emmission_workplace_air ewa ");
		...
		sql.append(" ORDER BY ewa.PERIOD desc ");
		List<EmissionWorkPlaceAir> list = (List<EmissionWorkPlaceAir>) executeQuery(sql.toString(), whList.toArray(), DB_ROW_MAPPER);
	...
	private RowMapper DB_ROW_MAPPER = new RowMapper(){
		@Override
		public EmissionWorkPlaceAir mapRow(ResultSet rs, int rowNum) throws SQLException {
			EmissionWorkPlaceAir m = new EmissionWorkPlaceAir();
	...
	public String insertData(EmissionWorkPlaceAir param) {
		StringBuilder sql = new StringBuilder();
		sql.append(" INSERT INTO she_emmission_workplace_air ( ");
		sql.append(" PERIOD          ");
	...
	public List<SystemParam> getSystemParam(String param) {
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT PARAM_CODE AS CODE , DESC_TH AS NAME, PARAM_TYPE AS TYPE FROM sys_param  ");
		sql.append(" WHERE PARAM_TYPE = '"+param+"' AND ACTIVE = 'Y' ORDER BY SORT_NO ASC");
		@SuppressWarnings("unchecked")
		List<SystemParam> list =(List<SystemParam>) executeQuery(sql.toString(), SYSTEM_MAPPER);
		return list;
	}
	...
	public String updateData(EmissionWorkPlaceAir param) {
		StringBuilder sql = new StringBuilder();
		sql.append(" UPDATE she_emmission_workplace_air SET ");
		sql.append("  SAMPLING1_1ST = ? 	");
		sql.append(" , SAMPLING1_2ND = ? 	");
		sql.append(" , SAMPLING1_3RD = ?	");
		sql.append(" , SAMPLING1_4TH = ? 	");
	...
	public int checkDuplicateData(EmissionWorkPlaceAir param) {
		List whList = new ArrayList<>();
		StringBuilder sql = new StringBuilder();
		sql.append(" SELECT ewa.* FROM she_emmission_workplace_air ewa ");
		sql.append(" WHERE 1 = 1 ");
